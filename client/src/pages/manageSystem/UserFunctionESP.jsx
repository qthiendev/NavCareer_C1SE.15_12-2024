...
function UserFunctionESP() {
    const [bannedData, setBannedData] = useState([]);
    const [error, setError] = useState('');
    const [loading, setLoading] = useState(true);
    const procedureNames = ['ReadUserCourses', 'CreateCourse', 'UpdateCourse', 'DeleteCourse'];
    ...
    const fetchBannedData = useCallback(async () => {
        ...
        setError('');
        try {
            const response = await axios.get('http://localhost:5000/admin/user/ban/read-esp', { withCredentials: true });
...
export default UserFunctionESP;
